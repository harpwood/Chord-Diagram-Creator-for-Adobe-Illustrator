...
function showMainWindow()
{
    ...
    var MainWindow = new Window("window", undefined, undefined, {su1PanelCoordinates: false, maximizeButton: false, minimizeButton: false, independent: false, borderless: false, resizeable: false}); 
        MainWindow.text = "Chord Diagram Creator"; 
    ...
    var DrawChordBtn = MainButtonGroup.add("button", undefined, undefined, {name: "DrawChordBtn"}); 
        ...
        DrawChordBtn.preferredSize.height = 40; 
            ...
            DrawChordBtn.onClick = function () 
            {
                ...
                nameOfChord = ChordNameInput.text == "" ? " " : ChordNameInput.text;
                numberOfStringsArg = parseInt(NumberOfStringsDropDown.selection.text.charAt(0));
                numberOfFretsArg = parseInt(NumberOfFretsDropDown.selection.text.charAt(0));

                fretPosArg = [];
                for (var i = 0; i < FretPositionsInput.length; i++)
                {
                    if (FretPositionsInput[i].text == "") fretPosArg.push("x");
                    else fretPosArg.push(FretPositionsInput[i].text);
                }

                fingerPosArg = [];
                for (var i = 0; i < FingersUsedInput.length; i++)
                {
                    fingerPosArg.push(FingersUsedInput[i].text);
                }

                //Pass the arguments and call the function to create the diagram via BridgeTalk
                try {
                    
                        //createChordDiagram(nameOfChord, numberOfStringsArg, numberOfFretsArg, fretPosArg, fingerPosArg);
                        btExecute('createChordDiagram', ['nameOfChord', 'numberOfStringsArg', 'numberOfFretsArg', 'fretPosArg', 'fingerPosArg']); 
                }catch(e){alert(ERROR_MSG + e, ERROR_TITLE);}

                if(prefs.canRepositionX)
                {
                    var sign = 1;
                    var spc = parseFloat(DiagramWidthInput.text) / 2;
                  
                    if(ReposSpacingInput.text != AUTO) spc = parseFloat(ReposSpacingInput.text);
                    if (spc < 0 ) sign = -1;
                    var xpos = parseFloat(PositioningXInput.text);
                    PositioningXInput.text = String(xpos + parseFloat(DiagramWidthInput.text) * sign + spc);
                ...
                {
                    var sign = 1;
                    var spc = parseFloat(DiagramHeightInput.text) / 2;
                    if(ReposSpacingInput.text != AUTO) spc = parseFloat(ReposSpacingInput.text);
                    if (spc < 0 ) sign = -1;
                    var ypos = parseFloat(PositioningYInput.text);
                    PositioningYInput.text = String(ypos + parseFloat(DiagramHeightInput.text) * sign + spc);
...
// Font: roman   Reflection: no   Adjustment: left   Stretch: no      Width: 200	 Text: Chords Engine

//   .oooooo.   oooo                                 .o8              oooooooooooo                         o8o                        
//  d8P'  `Y8b  `888                                "888              `888'     `8                         `"'                        
// 888           888 .oo.    .ooooo.  oooo d8b  .oooo888   .oooo.o     888         ooo. .oo.    .oooooooo oooo  ooo. .oo.    .ooooo.  
// 888           888P"Y88b  d88' `88b `888""8P d88' `888  d88(  "8     888oooo8    `888P"Y88b  888' `88b  `888  `888P"Y88b  d88' `88b 
// 888           888   888  888   888  888     888   888  `"Y88b.      888    "     888   888  888   888   888   888   888  888ooo888 
// `88b    ooo   888   888  888   888  888     888   888  o.  )88b     888       o  888   888  `88bod8P'   888   888   888  888    .o 
//  `Y8bood8P'  o888o o888o `Y8bod8P' d888b    `Y8bod88P" 8""888P'    o888ooooood8 o888o o888o `8oooooo.  o888o o888o o888o `Y8bod8P' 
//                                                                                             d"     YD                              
//                                                                                             "Y88888P'                              
                                                                                                                                   


function createChordDiagram(_chordNameUserInput,  _numStrings, _numFrets, _stringPositionUserInput, _fingerUsedUserInput)
{
    
    var _xx = parseFloat(prefs.x);
    var _yy = parseFloat(prefs.y);
    var _width = parseFloat(prefs.width);
    var _height = parseFloat(prefs.height);

    _strGridLinesThickness = prefs.stringThickness == AUTO ? undefined : parseFloat(prefs.stringThickness);
    _fretGridLinesThickness = prefs.fretThickness == AUTO ? undefined : parseFloat(prefs.fretThickness); 
    _nutThickness =  prefs.nutThickness == AUTO ? undefined : parseFloat(prefs.nutThickness);
    ...
    var _chordNameFontSize = prefs.chordNameFontSize == AUTO ? undefined : parseFloat(prefs.chordNameFontSize);
    var _chordNameFontH = parseFloat(prefs.chordNameFontH);
    var _chordNameFontV = parseFloat(prefs.chordNameFontV);
    if (_chordNameFontSize == undefined)  var fsize = _width / 5; else var fsize = _chordNameFontSize;
    
    var textRef = chordGroup.textFrames.add();      // text obj
    textRef.position = Array(_xx + _width /2 + _chordNameFontH, _yy + fsize * 1.2 + _chordNameFontV);
    textRef.contents = _chordNameUserInput;
    textRef.textRange.size = fsize;
    textRef.textRange.justification = Justification.CENTER;
    textRef.textRange.characterAttributes.textFont = textFonts.getByName(prefs.chordNameFont);
    
    var _fingersUsedFontSize = prefs.fingersUsedFontSize == AUTO ? undefined : parseFloat(prefs.fingersUsedFontSize); 
    var _fingersUsedFontH = parseFloat(prefs.fingersUsedFontH);
    var _fingersUsedFontV = parseFloat(prefs.fingersUsedFontV);
    ...
    if (_fingersUsedFontSize == undefined) var fsize = frtGap / 2; else var fsize = _fingersUsedFontSize;
    ...
    for (var f = 0; f < _numStrings; f++ )
    {
        ...
        fUsedTextRef.position = Array(_xx + (srtGap * f) + _fingersUsedFontH, _yy - _height + _fingersUsedFontV);          // pointText obj
        ...
        fUsedTextRef.textRange.characterAttributes.textFont = app.textFonts.getByName(prefs.fingersUsedFont);
    ...
    var _fretNumberFontSize = prefs.fingersUsedFontSize == AUTO ? undefined : parseFloat(prefs.fingersUsedFontSize);
    var _fretNumberFontH = parseFloat(prefs.fretNumberFontH);
    var _fretNumberFontV = parseFloat(prefs.fretNumberFontV);
    ...
    var neckGap = 0; 
    if (_fretNumberFontSize == undefined) var fsize = srtGap * .75; else var fsize = _fretNumberFontSize;
    ...
    if(Number(fingersCopy[0]) > 2) 
    {
        ...
        neckGapTextRef = chordGroup.textFrames.pointText([_xx - fsize * .75 + _fretNumberFontH, _fretNumberSizePos + _fretNumberFontV]);        // pointText obj
        ...
        neckGapTextRef.textRange.characterAttributes.textFont = app.textFonts.getByName(prefs.fretNumberFont); //TODO determine font
    ...
    else    // if the lowest fretted fret is not higher than the the 2rd fret....
    {
        ...
        if(Number(fingersCopy[fingersCopy.length - 1]) > _numFrets) 
        {
            ...
            neckGapTextRef = chordGroup.textFrames.pointText([_xx - fsize * .75 + _fretNumberFontH, _fretNumberSizePos + _fretNumberFontV]);
    ...
    var _fingersUsedFontSize = prefs.fingersUsedFontSize == AUTO ? undefined : parseFloat(prefs.fingersUsedFontSize);
    var _fretPositionsFontH = parseFloat(prefs.fretPositionsFontH);
    var _fretPositionsFontV = parseFloat(prefs.fretPositionsFontV);
    //Placing the fingering, frets wise -----
    var stringNumberTextRef=[];     // array of text oj

    if (_fingersUsedFontSize == undefined) var fsize = srtGap / 2; else var fsize = _fingersUsedFontSize;
    // positioning the fingering on diagram
    for (var s = 0; s < _numStrings; s++ )
    {   
        // if is closed string
        ...
        {
            stringNumberTextRef[s] = chordGroup.textFrames.pointText([_xx + (srtGap * s) - (srtGap / 8) + _fretPositionsFontH, _yy + srtGap / 8 + _fretPositionsFontV]);     // pointText obj
            ...
            stringNumberTextRef[s].textRange.characterAttributes.textFont = app.textFonts.getByName(prefs.fretPositionsFont); //TODO determine font
        } 
        // if is open string
        else  if (_stringPositionUserInput[s] == "o")
        {
            stringNumberTextRef[s] = chordGroup.textFrames.pointText([_xx + (srtGap * s) - (srtGap / 8) + _fretPositionsFontH, _yy + srtGap / 8 + _fretPositionsFontV]);
...
function showAboutDialog()
{
        ...
        var AboutText1 = AboutTextTopGroup.add("group"); 
            ...
            AboutText1.add("statictext", undefined, "for Adobe Illustrator " + VERSION + " ", {name: "AboutText1", multiline: true});  //here
